...
class SocketIOServerHandler:
    ...
    def new_connection(self, auth):
        ...
        if auth.get("payload"):
            return authorized_for_socket_io(request.sid, self.database, password=client_password, payload=True)
    ...
    def emit_screenshot_stream_for(self, payload_sid):
        ...
        if payload_sid in livestreams.keys():
            ...
            for browser_sid in livestreams:
                emit('newest_image_returned', {'data': self.database.get_last_screenshot()}, to=browser_sid)